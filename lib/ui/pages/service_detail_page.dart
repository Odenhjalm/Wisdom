...
class _ServiceDetailPageState extends State<ServiceDetailPage> {
  ...
  Future<void> _load() async {
    ...
    final s = await sb.app.from('services').select('id, provider_id, title, description, price_cents, active').eq('id', widget.id).maybeSingle();
    Map<String, dynamic>? prof;
    if (s != null) {
      prof = await sb.app.from('profiles').select('user_id, display_name, photo_url').eq('user_id', (s as Map)['provider_id']).maybeSingle();
    }
    if (!mounted) return;
    setState(() {
  ...
  Future<void> _buy() async {
    ...
    try {
      final order = await _payments.startServiceOrder(serviceId: id, amountCents: price);
  ...
  Widget build(BuildContext context) {
    ...
    if (_loading) {
      return const AppScaffold(title: 'Tjänst', body: Center(child: CircularProgressIndicator()));
    }
    final s = _service;
    if (s == null) {
      return const AppScaffold(title: 'Tjänst', body: Center(child: Text('Tjänst hittades inte')));
    }
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Text('${price.toStringAsFixed(2)} kr', style: t.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
                      const Spacer(),
                      ElevatedButton(
