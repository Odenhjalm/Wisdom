...
class TeacherRepo {
  ...
  TeacherRepo({SupabaseClient? client}) : client = client ?? supa;
  ...
  Map<String, dynamic> _mapModule(Map<String, dynamic> raw) {
    ...
    final lessons = (module['lessons'] as List? ?? [])
        ...
        .toList();
    lessons.sort((a, b) => (a['position'] as int).compareTo(b['position'] as int));
    final lesson = lessons.isNotEmpty ? lessons.first : null;
    final lessonMedia = (lesson?['lesson_media'] as List? ?? [])
        .map((e) => Map<String, dynamic>.from(e as Map))
        .toList();
    lessonMedia.sort((a, b) => (a['position'] as int).compareTo(b['position'] as int));
