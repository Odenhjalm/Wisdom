...
class _SignupPageState extends ConsumerState<SignupPage> {
                        ...
                        children: [
                          ...
                          Align(
                            ...
                            child: TextButton(
                              ...
                              child:
                                  const Text('Har du redan ett konto? Logga in'),
                            ),
                          ),
                          const SizedBox(height: 24),
                          const Divider(),
    ...
    try {
      ...
      if (res.user != null) {
        ...
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text('Konto skapat för $email')));
        context.go('/');
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Kontrollera din e-post för att bekräfta kontot.'),
          ),
        );
      }
  ...
  Future<void> _sendMagicLink(
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Magisk länk skickad till $email')),
      );
    } on AuthException catch (e) {
      _showError(e.message);
    } catch (e) {
  ...
  Future<void> _sendOtp(
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Engångskod skickad till $email')),
      );
    } on AuthException catch (e) {
      _showError(e.message);
    } catch (e) {
  ...
  void _showError(String message) {
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Fel: $message')),
    );
  }
}
