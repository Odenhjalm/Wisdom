...
class _SubscribeScreenState extends ConsumerState<SubscribeScreen> {
  ...
  Future<void> _activateFree() async {
    ...
    if (plan == null) {
      context.goSnack('Välj en plan först.');
      return;
    }
    final sb = ref.read(supabaseMaybeProvider);
    if (sb == null) {
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          OutlinedButton.icon(
                            onPressed:
                                (_selectedPlan == null || _loading) ? null : _preview,
                        ...
                        children: [
                          FilledButton(
                            onPressed: (_selectedPlan != null &&
                                    ...
                                    !_loading)
                                ? () async {
                                    final url = await getCheckoutUrl(_selectedPlan!);
                                    if (!mounted) return;
                                    context
                                        .goSnack('Öppna betalning i webbläsare: $url');
                                  }
                                : null,
...
class _PlanCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bgColor = selected ? scheme.primary.withValues(alpha: 0.08) : Colors.white;
