...
class _CourseEditorScreenState extends ConsumerState<CourseEditorScreen> {
  ...
  Future<void> _bootstrap() async {
    ...
    if (sb == null || sb.auth.currentUser == null) {
      context.ifMounted((c) => c.go('/login'));
      return;
    }
    try {
  ...
  Future<void> _pickAndUpload(String kind) async {
    final sb = ref.read(supabaseMaybeProvider);
    if (sb == null) {
      showSnack(context, 'Supabase saknas.');
      return;
    }
    final file = await openFile(
      acceptedTypeGroups: <XTypeGroup>[
        const XTypeGroup(
            label: 'Media',
            extensions: <String>['jpg', 'jpeg', 'png', 'mp4', 'mp3']),
      ],
    );
    if (file == null) return;
    ...
    try {
      ...
      showSnack(context, 'Uppladdning klar. Kom ihåg att spara.');
    } catch (e) {
      showSnack(context, 'Kunde inte ladda upp: $e');
    }
  }
  ...
  Future<void> _saveModule() async {
    ...
    try {
      ...
      await _loadModules();
      if (mounted) {
        showSnack(context, 'Sparat');
      }
    } catch (e) {
      if (mounted) {
        showSnack(context, 'Fel vid sparandet: $e');
      }
    } finally {
      if (mounted) {
  ...
  Future<void> _deleteModule(String id) async {
    try {
      ...
      showSnack(context, 'Kunde inte radera modul: $e');
    }
  }
                    ...
                    children: [
                      ...
                      if (_moduleType == 'text') ...[
                        TextField(
                          ...
                          maxLines: 5,
                          decoration:
                              const InputDecoration(labelText: 'Textinnehåll'),
                        ),
                        ...
                        const Text(
                            'Denna modul markerar en Quiz-sektion. Lägg frågor nedan.'),
                      ],
                      gap12,
                      Align(
                        alignment: Alignment.centerRight,
                        ...
                        ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return ListTile(
                              leading: Icon(icon),
                              title:
                                  Text('${m['title'] ?? type.toUpperCase()}'),
                              subtitle: Text('Pos ${m['position']}'),
                              trailing: IconButton(
                                icon: const Icon(Icons.delete_outline),
                                onPressed: () =>
                                    _deleteModule(m['id'] as String),
                              ),
                            );
                          },
                        ),
