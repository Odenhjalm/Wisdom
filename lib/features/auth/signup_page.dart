...
class _SignupPageState extends State<SignupPage> {
                    ...
                    child: AutofillGroup(
                      child: Column(
                        ...
                        children: [
                          ...
                          gap12,
                          OutlinedButton(
                            onPressed: _busy ? null : () => _sendMagicLink(context),
  ...
  Future<void> _signUp(BuildContext context) async {
    ...
    try {
      ...
      if (response.session != null) {
        gate.allow();
        if (!mounted) return;
        context.goSnack('Konto skapat. Inloggad som $email');
        if (!mounted) return;
        context.go('/');
      } else {
        if (!mounted) return;
        context.goSnack('Vi har skickat en bekräftelselänk till din e-post.');
      }
  ...
  Future<void> _sendMagicLink(BuildContext context) async {
    ...
    if (!_isValidEmail(email)) {
      context.goSnack('Ange en giltig e-postadress först.');
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      context.goSnack('Magisk länk skickad.');
    } on AuthException catch (e) {
      if (!mounted) return;
      context.goSnack(e.message);
    } catch (e) {
